...
return new class extends Migration
{
    ...
    {
        Schema::create('roc_iec_verifications', function (Blueprint $table) {
            $table->id();
            $table->foreignId('user_id')->constrained('registrations')->onDelete('cascade');
            $table->string('import_export_code')->index();
            ...
            $table->index(['user_id', 'import_export_code']);
            $table->index('status');
        });
    }
    ...
    {
        Schema::dropIfExists('roc_iec_verifications');
    }
};
